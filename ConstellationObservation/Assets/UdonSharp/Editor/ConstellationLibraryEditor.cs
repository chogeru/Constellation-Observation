...
{
    /// <summary>
    /// Shows the constellation name, its target object, and its narration clip
    /// side-by-side on one row, instead of three separate array lists that are
    /// easy to mismatch by index.
    /// </summary>
    [CustomEditor(typeof(ConstellationLibrary))]
    public class ConstellationLibraryEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("Constellations (presentation order)", EditorStyles.boldLabel);
            ...
            EditorGUILayout.Space();
            if (GUILayout.Button("Add Constellation"))
            {
                int i = count;
