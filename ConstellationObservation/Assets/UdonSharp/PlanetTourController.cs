...
{
    /// <summary>
    /// Walks the player through the planets one at a time: the current planet glides in
    /// front of the player, highlighted (a glow light gently pulses, with a bright flash on
    /// arrival), and named on a floating label while its narration plays. When it's done it
    /// glides back to its real orbital position and the next planet takes its place, before
